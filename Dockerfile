...
RUN apt-get update && \
    ...
    # Kubernetes repo
    #
    curl -fsSL https://pkgs.k8s.io/core:/stable:/${versionKubectl}/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg && \    
    echo "deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/${versionKubectl}/deb/ /" | sudo tee /etc/apt/sources.list.d/kubernetes.list && \
    #
    # Github shell
